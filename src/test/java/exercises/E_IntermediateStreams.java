...
public class E_IntermediateStreams {
    ...
    public void e1_stringsToCharacters() {
        ...
        List<Character> result = input.stream().flatMap(s -> s.chars().mapToObj(x -> (char) x)).collect(Collectors.toList());
    ...
    public void e2_listOfAllWords() throws IOException {
        List<String> output = reader.lines().flatMap(s -> SPLIT_PATTERN.splitAsStream(s)).collect(Collectors.toList());
    ...
    public void e3_longLowerCaseSortedWords() throws IOException {
        List<String> output = reader.lines().flatMap(s -> SPLIT_PATTERN.splitAsStream(s)).filter(s -> s.length() >= 8).map(String::toLowerCase)
    ...
    public void e4_longLowerCaseReverseSortedWords() throws IOException {
        List<String> result = reader.lines().flatMap(s -> SPLIT_PATTERN.splitAsStream(s)).
                filter(s -> s.length() >= 8).map(String::toLowerCase).sorted(Comparator.reverseOrder()).collect(Collectors.toList());
    ...
    public void e5_sortedLowerCaseDistinctByLengthThenAlphabetically() throws IOException {
        List<String> result = reader.lines().flatMap(s -> SPLIT_PATTERN.splitAsStream(s))
                .map(String::toLowerCase).distinct().sorted(Comparator.comparing(String::length)
    ...
    public void e6_bigFactorial() {
        BigInteger result = LongStream.rangeClosed(1, 21).mapToObj(BigInteger::valueOf).reduce(BigInteger.ONE, BigInteger::multiply);
    ...
    public void e7_getLastWord() throws IOException {
        String result = reader.lines().flatMap(s -> SPLIT_PATTERN.splitAsStream(s)).reduce((start, end) -> end).orElse("");
